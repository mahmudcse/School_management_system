...
<div class="form-group">
		...
		<div class="col-sm-5">
			<select name="subject_id" class="form-control selectboxit">
				...
				<?php 
					$subjects = $this->db->get_where('course' , array('class_id' => $class_id))->result_array();
					foreach($subjects as $subject):
				?>
				<option value="<?php echo $subject['course_id'];?>"
					<?php if($subject['course_id'] == $subject_id) echo 'selected';?>>
						<?php echo $subject['tittle'];?>
					</option>
				<?php endforeach;?>
			</select>
		</div>
