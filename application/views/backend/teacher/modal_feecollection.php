...
<?php echo form_open(base_url() . 'index.php?teacher/save_student_fee/create' , array('id' => 'myform'));?>	
		...
		<table class="table">
			...
			<tbody>
				...
				<tr>
					...
					<td>Date: <input type="text" class="" name="timestamp" value="<?php echo date('d-m-Y');?>" style="padding-left: 15px" required readonly>
					</td>
				...
				<tr>
					...
					<td style="text-align: left"><?php echo $groupClass[0][value].": ". $studentinfo['class']?></td>
...
?>
    ...
    <tr>
      ...
      <th style="text-align: right; padding-right: 10px"><input type="text" class="amount" readonly value="<?php echo number_format((float)$row['amount'], 2, '.', '') ?>" style="text-align: right; padding-right: 10px; ">TK.
...
?>
	...
	<div>
		...
		<input type="checkbox" name="msms" checked="checked" />&nbsp;&nbsp;Mobile SMS
 ...
 <script type="text/javascript">
    ...
    function calculate(item){
        ...
        if(item.checked){
        	...
        	var countValue  = item.value;
        	var countValue  = parseInt(countValue);
        	var tempamount  = $('input.amount').eq(countValue).val();
        	var amount = parseInt(tempamount);
            total+= amount;

        }else{
        	var subValue  = item.value;
        	var subValue  = parseInt(subValue);
        	var tsubamount  = $('input.amount').eq(subValue).val();
        	var subamount = parseInt(tsubamount);
         	total-= subamount;
        }
