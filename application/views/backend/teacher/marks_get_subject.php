...
<div class="col-md-3">
	...
	<label class="control-label" style="margin-bottom: 5px;"><?php echo get_phrase('subject');?></label>
		<select name="subject_id" id="subject_id" class="form-control selectboxit">
			<option value=""><?php echo get_phrase('select_one')?></option>
			<?php 
				$subjects = $this->db->get_where('course' , array(
					'class_id' => $class_id))->result_array();
				foreach($subjects as $row):
			?>
			
			<option value="<?php echo $row['course_id'];?>"><?php echo $row['tittle'];?></option>
			<?php endforeach;?>
		</select>
	</div>
</div>
