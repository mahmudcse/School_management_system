...
/*        $results = $this->db->get_where('vstudent_result_summery', array('student_id'=>$student_id))->result_array();
    ...
    public function get_students_mark_dashboard($course_id) {

        if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
        $student_id = $this->session->userdata('student_id');
        $course_name = $this->db->get_where('course', array('course_id' => $course_id))->row()->title;
        $results = $this->db->get_where('vstudent_result_summery', array('student_id'=>$student_id, 'course_name'=>$course_name))->result_array();
        
        $page_data['results'] = $results;
        $page_data['course_id'] = $course_id;
        $page_data['course_name'] = $course_name;
        $page_data['page_name']  = 'dashboard';
        $page_data['page_title'] = get_phrase('student_dashboard');
        $this->load->view('backend/index', $page_data);
    ...
    function document_($do = '', $document_id = '')
    {
        if ($this->session->userdata('student_login') != 1)
            redirect('login', 'refresh');
    ...
    function message($param1 = 'message_home', $param2 = '', $param3 = '') {
        ...
        if ($param1 == 'send_new') {
            ...
            redirect(base_url() . 'index.php?'.$account_type.'/message/message_read/' . $message_thread_code, 'refresh');
        }
    ...
    function document($do = '', $documentId = '')
    {
    	...
    	if ($this->session->userdata('student_login') != 1)
    		...
    		$students  = $this->db->get_where('student', array("student_id"=>$studentId))->result_array();
    		...
    		$page_data['documents']  = $this->db->get_where('vDocShare', array('type'=>'CLASS', 'shareToId'=>$classId))->result_array();
